...
class GetTicketInfoSpec extends AnyWordSpec with Matchers {
  ...
  "getTicketInfo" should {
    "return a complete ticket info when all futures are successful" in {
      ...
      ticketInfo.event.isEmpty should be(false)
      ticketInfo.event.foreach(event => event.name should be("Quasimoto"))
      ticketInfo.travelAdvice.isEmpty should be(false)
      ticketInfo.suggestions.map(_.name) should be(Seq("Madlib", "OhNo", "Flying Lotus"))
    }
    "return an incomplete ticket info when getEvent fails" in {
      ...
      ticketInfo.event.isEmpty should be(true)
      ticketInfo.travelAdvice.isEmpty should be(true)
      ticketInfo.suggestions.isEmpty should be(true)
    }
  }
}
...
trait MockWebServiceCalls extends WebServiceCalls {
      ...
      if (ticketNr == "1234") {
        TicketInfo(
          ...
          event = Some(
            Event(
              "Quasimoto",
              Location(4.324218908d, 53.12311144d),
              new DateTime(2013, 10, 1, 22, 30),
            )
          ),
        )
  ...
  def callTrafficService(
      origin: Location,
      destination: Location,
      time: DateTime,
  ): Future[Option[RouteByCar]] = {
    Future {
